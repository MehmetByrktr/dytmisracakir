...
export default function MenuFilter({ menus, labels }: { menus: MenuPlan[]; labels?: { searchPlaceholder?: string; filterButton?: string; categoryButton?: string } }) {
  ...
  return <div>
    <form onSubmit={submit} className="mb-10 rounded-xl2 border border-ink/[0.07] bg-white/55 p-3 shadow-card backdrop-blur-sm sm:p-4"><div className="grid gap-3 md:grid-cols-[1fr_240px_auto]">
      <label className="relative block"><span className="sr-only">Menülerde ara</span><Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-clay" /><input value={queryDraft} onChange={(e) => setQueryDraft(e.target.value)} placeholder={labels?.searchPlaceholder || 'Menülerde ara...'} className="h-12 w-full rounded-full border border-ink/10 bg-cream/70 pl-11 pr-4 text-sm text-ink outline-none transition focus:border-clay focus:ring-2 focus:ring-clay/10" /></label>
    ...
    </div>{hasFilter && <div className="mt-3 flex items-center justify-between px-2 text-xs text-ink-faint"><span>{filtered.length} menü bulundu</span><button type="button" onClick={clear} className="inline-flex items-center gap-1 font-semibold text-clay-deep hover:text-clay"><X className="h-3.5 w-3.5" />Filtreyi temizle</button></div>}</form>
    {filtered.length === 0 ? <p className="py-16 text-center text-sm text-ink-faint">Aramanıza uygun menü bulunamadı.</p> : <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{filtered.map((menu, i) => <RevealOnScroll key={menu.slug} delay={(i % 3) * 0.08}><MenuCard menu={menu} /></RevealOnScroll>)}</div>}
  </div>;
}
