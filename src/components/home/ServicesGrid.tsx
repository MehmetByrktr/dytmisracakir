...
export default function ServicesGrid({ services, siteData = defaultSite }: { services: Service[]; siteData?: typeof defaultSite }) {
  ...
  return <section className="section-pad bg-cream"><div className="container-site">
    ...
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{services.map((service, i) => { const Icon = iconMap[service.icon]; return <RevealOnScroll key={service.slug} delay={(i % 3) * 0.08}><Link href={`/danismanliklar/${service.slug}`} className="paper-card group flex h-full flex-col !border-clay/20 !bg-gradient-to-br !from-white/95 !via-white/90 !to-clay-soft/25 p-7 transition-all duration-300 hover:-translate-y-1.5 hover:!border-clay/35 hover:shadow-card-hover">
      ...
      <h3 className="mt-5 font-display text-xl text-ink">{service.title}</h3><p className="mt-2 flex-1 text-sm leading-relaxed text-ink-soft">{service.shortDescription}</p>
