...
export default async function ServiceDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="pb-24 pt-32 sm:pt-40"><div className="container-site">
    ...
    <RevealOnScroll delay={0.1}><div className="paper-card sticky top-28 p-7"><div className="aspect-video overflow-hidden rounded-lg bg-sage-50">{service.image ? <img src={service.image} alt={service.title} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center px-6 text-center font-mono text-[11px] uppercase tracking-wide text-sage-600">Görsel alanı</div>}</div><dl className="mt-6 space-y-4 text-sm"><div className="flex items-center justify-between border-b border-ink/[0.06] pb-4"><dt className="flex items-center gap-2 text-ink-faint"><Clock className="h-4 w-4" /> Süre</dt><dd className="font-medium text-ink">{service.duration}</dd></div><div className="flex items-center justify-between"><dt className="flex items-center gap-2 text-ink-faint"><MapPin className="h-4 w-4" /> Format</dt><dd className="font-medium text-ink">{service.format}</dd></div></dl><Button href="/randevu" className="mt-7 w-full" icon={<ArrowRight className="h-4 w-4" />}>Bu Danışmanlık İçin Randevu Al</Button></div></RevealOnScroll></div>
