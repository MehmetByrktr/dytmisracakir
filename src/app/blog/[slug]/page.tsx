...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const post = (await getContent()).blogPosts.find((item) => item.slug === slug && item.status !== 'draft');
  if (!post) return {};
  return {
    title: post.title,
    ...
    <div className="pb-24 pt-32 sm:pt-40">
      ...
      <div className="container-site">
        <nav aria-label="breadcrumb" className="mb-8 flex items-center gap-1.5 text-xs text-ink-faint">
          ...
          <ChevronRight className="h-3 w-3" />
          <span className="line-clamp-1 text-ink-soft">{post.title}</span>
        </nav>

        <div className="mx-auto max-w-3xl">
          <RevealOnScroll>
            <span className="eyebrow">{post.category}</span>
            <h1 className="mt-3 text-balance font-display text-3xl font-medium leading-tight text-ink sm:text-4xl">{post.title}</h1>
            <div className="mt-5 flex flex-wrap items-center justify-between gap-4 border-b border-ink/[0.06] pb-6">
              <div className="flex flex-wrap items-center gap-3 text-sm text-ink-faint">
                ...
                <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {post.readMinutes} dk</span>
              </div>
              <div className="flex flex-wrap items-center gap-3">
                <BlogEngagement slug={post.slug} initialViews={post.views} initialLikes={post.likes} />
                <ShareButtons
                  url={url}
                  instagramUrl={site.social.instagram}
                  tiktokUrl={site.social.youtube}
                />
              </div>
            </div>
          </RevealOnScroll>

          <div className="relative mt-8 aspect-[16/9] overflow-hidden rounded-xl2 bg-sage-50">
            {post.coverImage ? (
              <Image src={post.coverImage} alt={post.title} fill sizes="(max-width: 768px) 100vw, 768px" className="object-cover" />
            ) : (
              <div className="flex h-full items-center justify-center font-mono text-xs uppercase tracking-wide text-sage-600">Kapak görseli</div>
            )}
          </div>
      ...
      {related.length > 0 && (
        <div className="container-site mt-24">
          <h2 className="font-display text-2xl text-ink">Benzer yazılar</h2>
