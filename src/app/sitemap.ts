...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const menuRoutes = menus.filter((item) => item.status !== 'draft').map((item) => ({ url: `${baseUrl}/menuler/${item.slug}`, lastModified: new Date(), changeFrequency: 'monthly' as const, priority: 0.5 }));
